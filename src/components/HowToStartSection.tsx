  ...
  return (
    <section className="bg-how-to-start py-14 md:py-24">
      <div className="relative container flex flex-col items-center justify-center py-4">
        <p className="z-10 text-center text-base text-secondary font-semibold uppercase text-greenFM">How to start</p>
        ...
        {[1, 2, 3, 4].map((step) => (
            <div key={step} className="flex flex-col gap-2 p-4 bg-tertiary md:bg-transparent">
              <div className="flex items-center flex-row md:flex-col gap-2">
                <span className="text-secondary text-lg md:text-4xl font-bold text-greenFM">{step}</span>
              ...
              <p className="text-sm md:text-base md:text-center text-grey">
                {step === 1 && "We don't collect any fee for listing livestock."}
                {step === 2 && 'We strive for providing the highest security.'}
                {step === 3 && 'We help you create a listing including breed, age, weight.'}
                {step === 4 && 'Thousands of nationwide buyers searching for livestock every day!'}
              </p>
            </div>
          ))}
        </div>
